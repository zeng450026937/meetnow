...
const Factory = (root) => {
  ...
  return function (userAgent) {
    ...
    this.isEdge = match.Edge;
    this.isChrome = match.Chrome && !this.isEdge;
    this.isFirefox = match.Firefox;

    if (_window.chrome) {
      const chrome_version = u.replace(/^.*Chrome\/([\d]+).*$/, '$1');

      this.chromeVersion = chrome_version;
    ...
    const version = {
      ...
      '360SE' : function () {
        ...
        return hashTable[chrome_version] || '';
      },
      '360EE' : function () {
        const hashTable = {
      ...
      LBBROWSER() {
        const hashTable = {
          57 : '6.5', 49 : '6.0', 46 : '5.9', 42 : '5.3', 39 : '5.2', 34 : '5.0', 29 : '4.5', 21 : '4.0',
        };
      ...
      '2345Explorer' : function () {
        const hashTable = { 69: '10.0', 55: '9.9' };
        const chrome_version = navigator.userAgent.replace(/^.*Chrome\/([\d]+).*$/, '$1');

        return hashTable[chrome_version] || u.replace(/^.*2345Explorer\/([\d.]+).*$/, '$1').replace(/^.*Mb2345Browser\/([\d.]+).*$/, '$1');
      },
      TheWorld() {
    ...
    else if (_this.browser === 'Chrome' && Number.parseInt(_this.version, 10) > 27) {
      _this.engine = 'Blink';
    }
    else if (_this.browser === 'Opera' && Number.parseInt(_this.version, 10) > 12) {
      _this.engine = 'Blink';
    }
